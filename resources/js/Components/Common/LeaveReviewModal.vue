...
const submit = async () => {
    ...
    if (rating.value === 0) {
        errors.value.rating = 'Lūdzu, izvēlieties vērtējumu.';
        return;
    }
    ...
    try {
        ...
        if (e.response?.status === 422) {
            ...
            toast.error(e.response.data.message ?? 'Lūdzu, pārbaudiet formu.');
        } else {
            toast.error(e.response?.data?.message ?? 'Notika kļūda.');
        }
    } finally {
...
<template>
    <Modal :show="show" @close="close" maxWidth="md">
        <div class="bg-navy px-6 py-4 flex items-center justify-between">
            <div>
                <h2 class="text-base font-bold text-white">Atstāt atsauksmi</h2>
                <p class="text-xs text-white/50 mt-0.5">Par: {{ revieweeName }}</p>
            </div>
        ...
        <div class="p-6 space-y-5">
            <!-- Star rating -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Vērtējums</label>
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Pastāstiet par sadarbības pieredzi..."
                ></textarea>
        ...
        <div class="px-6 pb-6 flex items-center justify-end gap-3 border-t border-gray-100 pt-4">
            ...
            >
                ...
                {{ processing ? 'Saglabā...' : 'Iesniegt atsauksmi' }}
            </button>
        </div>
    </Modal>
</template>
