...
<template>
    <Modal :show="show" @close="emit('close')" maxWidth="sm">
        <div v-if="proposal" class="p-6">
            <h3 class="text-base font-bold text-navy mb-3">{{ t('proposals.withdraw_title') }}</h3>
            <p class="text-sm text-gray-600 mb-5">
                {{ t('proposals.withdraw_desc', { amount: formatMoney(proposal?.amount) }) }}
            </p>
            <div class="flex justify-end gap-3">
                ...
                >
                    {{ t('proposals.withdraw_confirm_btn') }}
                </button>
            </div>
        </div>
    </Modal>
