...
type ChildCategoryWithCount = Omit<Category, 'children'> & { services_count: number };
...
const totalServices = computed(() =>
    props.categories.reduce((sum, c) => {
        const childSum = c.children?.reduce((s, ch) => s + ch.services_count, 0) ?? 0;
        return sum + c.services_count + childSum;
    }, 0)
);

function categoryTotal(c: CategoryWithCount): number {
    return c.services_count + (c.children?.reduce((s, ch) => s + ch.services_count, 0) ?? 0);
}
                    ...
                    >
                        ...
                        >
                            ...
                            <Link
                                ...
                                :key="child.id"
                                :href="route('seeker.services.index', { category_id: child.id })"
                            ...
                            >
                                ...
                                <span class="text-xs text-gray-400">{{ child.services_count }}</span>
                            </Link>
                        </div>
                    </div>
